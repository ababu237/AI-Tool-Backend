...
MASTER_API_KEY = CONFIG.get('MASTER_API_KEY') or os.getenv('MASTER_API_KEY')  # separate key to gate endpoints

REQUIRE_API_KEY = bool(MASTER_API_KEY)  # Only enforce if provided

API_KEY_HEADER_NAME = 'x-api-key'

def verify_api_key(x_api_key: Optional[str] = None):  # Will be injected manually for clarity
    if REQUIRE_API_KEY and x_api_key != MASTER_API_KEY:
...
def translate_text_unified(text: str, target_lang: str) -> str:
    ...
    try:
        ...
        if _legacy_translate_text and src != 'auto':
            try:
                return _legacy_translate_text(text, target_lang)  # some legacy signatures differ; best effort
            except Exception:
                pass
...
# App Initialization
# ----------------------------------------------------------------------------
app = FastAPI(title='Master Healthcare AI Backend', description='Unified API exposing clinical chat, document Q&A, CSV agent, organ analyzer, transcription, and translation endpoints.')
...
async def test_transcription_endpoint(output_language: str = Form('en'), x_api_key: Optional[str] = Form(None)):
    verify_api_key(x_api_key)
...
async def test_organ_analyzer_endpoint(organ: str = Form('lung'), input_language: str = Form('en'), output_language: str = Form('en'), x_api_key: Optional[str] = Form(None)):
    verify_api_key(x_api_key)
...
async def translate_text_endpoint(text: str = Form(...), target_language: str = Form('en'), x_api_key: Optional[str] = Form(None)):
    verify_api_key(x_api_key)
    try:
...
async def text_to_speech_endpoint(text: str = Form(...), x_api_key: Optional[str] = Form(None)):
    verify_api_key(x_api_key)
    try:
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run('backend_api.master_backend_api:app', host='0.0.0.0', port=8000, reload=True)
